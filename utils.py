...
def node_norm_to_edge_norm(g, norm):
    ...
    norm = g.edata.pop('norm').squeeze().to("cuda")
    return norm
...
def com_mult(a, b):
    ...
    r2, i2 = b.real, b.imag
    # return torch.stack([r1 * r2 - i1 * i2, r1 * i2 + i1 * r2], dim=-1)
    return torch.complex(r1 * r2 - i1 * i2, r1 * i2 + i1 * r2)
...
def ccorr(a, b):
    return torch.fft.irfft(com_mult(conj(torch.fft.rfft(a, 1)), torch.fft.rfft(b, 1)), a.shape[-1])
